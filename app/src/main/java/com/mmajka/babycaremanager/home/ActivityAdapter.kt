...
class ActivityAdapter(val actions: LiveData<List<BasicActionEntity>>): RecyclerView.Adapter<ActivityViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ActivityViewHolder, position: Int) {
        val bind = actions.value!!.get(position)
        val image = holder.image
        when(actions.value!!.get(position).title){
            "Diaper" -> {
